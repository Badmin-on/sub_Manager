...
const ShortcutGrid: React.FC<ShortcutGridProps> = ({ shortcuts, categories, onDelete, onEdit, isSignedIn, isLoading, isConfigured }) => {
  const { t } = useLanguage();
  ...
  if (isLoading) {
    return (
        <div className="text-center py-20">
            <h3 className="mt-2 text-sm font-medium text-gray-900">{t('shortcutGrid.loading')}</h3>
        </div>
    )
  }
  ...
  if (shortcuts.length === 0) {
      ...
      if (!isConfigured) {
          heading = t('shortcutGrid.localModeTitle');
          paragraph = t('shortcutGrid.localModeMessage');
      } else if (!isSignedIn) {
          heading = t('shortcutGrid.signInPrompt');
          paragraph = t('shortcutGrid.signInToSync');
      } else {
          heading = t('shortcutGrid.noShortcuts');
          paragraph = t('shortcutGrid.getStarted');
      }
