-- Supabase 데이터베이스 테이블 생성 및 설정 스크립트
...
-- 1. Categories 테이블 생성 (외래 키 제약 조건 없이)
CREATE TABLE categories (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    name TEXT NOT NULL,
    user_id TEXT NOT NULL DEFAULT 'fixed-user-12345',
...
-- 4. RLS 정책 생성 (Categories) - 기존 정책 삭제 후 재생성
...
-- 5. RLS 정책 생성 (Shortcuts) - 기존 정책 삭제 후 재생성
DROP POLICY IF EXISTS "Users can CRUD their own shortcuts" ON shortcuts;
